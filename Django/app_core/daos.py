...
def make_category_tree(status=False, board_id=None):
    ...
    category_dict = {
        ...
        'children': [],
        'post_count': 0 if status else models.POST.objects.filter(place_info__categories__id__in=[category.id], place_info__status__in=['active', 'pending', 'ad'], boards__id=board_id if board_id else None).count(),
